...
def irt(
    ...
    model (str): IRT模型类型，支持'2pl'、'grm_stand'、'grm_step'
    ...
    if n_categories is None and model != '2pl':
        raise ValueError(
            "当model为'grm_stand'或'grm_step'时，n_categories参数不能为空，请传入每个项目的类别数。\n"
        )
    ...
    if missing_flag and model=='grm_standard':
        print(
            "警告: 数据中部分作答数量过少，采用标准grm模型处理可能影响IRT估计结果。\n" +
            "\n".join(missing_list) +
            "请考虑采用grm_stepwise模型进行估计。"
        )
    if n_quadrature < 1:
    ...
    if model.lower() == '2pl':
        ...
        raise ValueError(f"不支持的IRT模型: {model}，请从 '2pl', 'grm_stand', 'grm_step' 中选择。")
